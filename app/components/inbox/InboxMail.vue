...
<template>
  <UDashboardPanel id="inbox-2" data-testid="email-details-panel">
    <UDashboardNavbar :title="mail.subject" :toggle="false" data-testid="email-navbar">
      <template #leading>
    ...
    <div class="pb-4 px-4 sm:px-6 shrink-0">
      <UCard variant="subtle" class="mt-auto" :ui="{ header: 'flex items-center gap-1.5 text-dimmed' }"
        ...
        <template #header>
          ...
          <span class="text-sm truncate" data-testid="reply-header">
            Reply to {{ mail.from.name }} ({{ mail.from.email }})
          </span>
        </template>

        <form @submit.prevent="onSubmit" data-testid="reply-form">
          <UTextarea v-model="reply" color="neutral" variant="none" required autoresize
            ...
            :ui="{ base: 'p-0 resize-none' }" data-testid="reply-textarea" />

          <div class="flex items-center justify-between">
            <UTooltip text="Attach file">
              <UButton color="neutral" variant="ghost" icon="i-lucide-paperclip" data-testid="attach-file-button" />
            </UTooltip>

            <div class="flex items-center justify-end gap-2">
              <UButton color="neutral" variant="ghost" label="Save draft" data-testid="save-draft-button" />
